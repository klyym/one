...
然后在左侧菜单的 **Table Editor** 中，你应该能看到以下 8 张表：
...
✅ clients           - 客户
✅ designers         - 设计师
✅ projects          - 项目（设计跟进/工程施工）
✅ project_phases    - 项目阶段（已弃用，保留兼容）
✅ follow_ups        - 跟进记录
✅ health_check      - 健康检查
```
...
3. **clients** - 客户信息
4. **designers** - 设计师信息
5. **projects** - 项目信息（含 stage 设计/施工、造价、跟进记录 updates jsonb）
6. **project_phases** - 项目阶段进度（已弃用，保留兼容）
7. **follow_ups** - 跟进记录
8. **health_check** - 系统健康检查

> 说明：`design_cases`（设计案例）表已随案例展示模块删除，不再创建。
> 如需保留现有数据升级，请使用 `scripts/migrate-database.sql`（增量脚本）。
