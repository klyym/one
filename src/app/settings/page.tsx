          ...
          <div className="space-y-4">
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                从 Supabase 云数据库加载所有数据并覆盖本地数据。这将替换浏览器中存储的所有项目、客户、设计师等信息。
              </p>
            </div>
            <Button
              variant="outline"
              onClick={async () => {
                if (confirm('确定要从 Supabase 重新加载数据吗？这将覆盖本地所有数据！')) {
                  try {
                    ...
                    const { clientService, designerService, projectService, followUpService } = services;
                    ...
                    if (supabaseFollowUps && supabaseFollowUps.length > 0) {
                      const localFollowUps = supabaseFollowUps.map((f: any) => ({
                        ...
                        designerId: f.designer_id || f.followed_by,
                        type: f.type,
                        content: f.content,
        ...
        <CardContent>
          <div className="space-y-4">
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                删除浏览器 localStorage 中的所有数据（项目、客户、设计师等）。
