...
export const projects = pgTable(
	"projects",
	{
		...
		stage: varchar("stage", { length: 20 }).notNull().default("design"), // design 设计跟进 / construction 工程施工
		priority: varchar("priority", { length: 50 }).notNull().default("medium"), // low, medium, high
		budget: numeric("budget", { precision: 12, scale: 2 }),
		contract_amount: numeric("contract_amount", { precision: 12, scale: 2 }), // 合同额（元）
		actual_cost: numeric("actual_cost", { precision: 12, scale: 2 }), // 实际成本（元）
		area: numeric("area", { precision: 10, scale: 2 }), // 面积（平方米）
		address: varchar("address", { length: 500 }),
		style: varchar("style", { length: 100 }), // 设计风格
		overall_progress: integer("overall_progress").default(0), // 总体进度 0-100（手动维护）
		updates: jsonb("updates").default([]), // 项目跟进记录 [{ id, date, content }]
